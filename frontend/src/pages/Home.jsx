...
const Home = () => {
  ...
  return (
    <div className="bg-black min-h-screen text-white relative selection:bg-gold selection:text-black">
      ...
      {/* 1. HERO SECTION */}
      <section className="relative h-[85vh] flex items-center justify-center overflow-hidden z-10 border-b border-gold/20">
        <div className="absolute inset-0 bg-black/70 z-10" />
        <img src="https://images.unsplash.com/photo-1560066984-138dadb4c035?auto=format&fit=crop&w=1920&q=80" alt="Luxury Spa" className="absolute inset-0 w-full h-full object-cover grayscale" />

        <div className="relative z-20 text-center px-4 max-w-6xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}>
            <span className="text-gold font-black uppercase tracking-[0.8em] text-[10px] mb-6 block">The Private Collection</span>
            ...
            <p className="text-sm md:text-xl font-light tracking-[0.4em] text-white/80 uppercase mb-12">Unrivaled Mastery • Absolute Luxury</p>
            <div className="flex justify-center">
              <a href="#menu" className="btn-gold px-12 py-5 text-xs tracking-widest">Explore Our Menu</a> 
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section id="menu" className="relative z-10 py-24 md:py-32 max-w-7xl mx-auto px-4">
        <div className="text-center mb-16 md:mb-24">
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <h2 className="text-4xl md:text-7xl font-serif font-bold uppercase tracking-widest text-gold mb-4 text-shadow-lg">The Collection</h2>
              <p className="text-gold/60 uppercase tracking-[0.4em] text-[10px] font-black italic">Select a category to discover perfection</p>
            </motion.div>
        </div>
            ...
            {categories.map((cat) => (
                <button
                    ...
                    className={`px-6 md:px-12 py-4 text-[10px] font-black uppercase tracking-widest border transition-all duration-500 ${
                        activeCategory === cat ? 'bg-gold text-black border-gold shadow-[0_10px_40px_rgba(212,175,55,0.3)]' : 'border-gold/20 text-gold/40 hover:border-gold/60 hover:text-gold'
                    }`}
                >
                    {cat}
                </button>
                            ...
                            >
                                <div className="space-y-3 relative z-10">
                                    <div className="flex justify-between items-start gap-4">
                                      <h3 className="text-xl md:text-3xl font-serif font-bold text-white group-hover:text-gold transition-colors leading-tight">{service.name}</h3>
            ...
            {[
              ...
              >
                ...
                <div className="space-y-1">
                  <p className="text-[10px] text-white/40 uppercase font-black tracking-widest">{offer.guests}</p>
                  <p className="text-3xl font-serif font-black text-gold">{offer.discount}</p>
                </div>
                <p className="text-[10px] text-white/60 italic uppercase tracking-widest leading-relaxed pt-6 border-t border-gold/10">{offer.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
                ...
                >
                    ...
                    <div className="flex-1 p-8 md:p-16 flex flex-col justify-between overflow-y-auto custom-scrollbar">
                        ...
                        <div className="space-y-12">
                            ...
                            >
                                <div className="space-y-1">
                                    <p className="text-[10px] uppercase text-gold/40 tracking-[0.3em] font-black">Charge</p>
                                    <p className="text-4xl md:text-6xl font-serif font-black text-gold">KSh {selectedService.price}</p>
                                </div>
                            </motion.div>
                            ...
                            >
                                <p className="text-white/80 font-light text-xl md:text-2xl leading-relaxed italic border-l-2 border-gold/40 pl-8">
                                    "{selectedService.description || `A handcrafted beauty ritual designed by our master technicians to enhance your natural radiance.`}"
                                </p>
                                
                                <div className="flex flex-wrap gap-4 pt-4">
                                   {['Certified Master Techs', 'Organic Pure Rituals', 'Luxury Environment'].map((tag) => (
                                     <span key={tag} className="px-5 py-3 border border-gold/20 bg-gold/5 text-[9px] uppercase tracking-[0.2em] text-gold font-black">
                                       {tag}
                                     </span>
                                   ))}
                                </div>
                        ...
                        >
                            <Link 
                                ...
                                onClick={() => setSelectedService(null)}
                            >
                                <span className="relative z-10 text-black text-sm md:text-base font-black uppercase tracking-[0.5em]">Confirm Reservation</span>
      ...
      <section className="py-32 border-t border-gold/20 bg-black">
        <div className="max-w-5xl mx-auto flex flex-wrap justify-center gap-16 md:gap-32">
            <div className="flex flex-col items-center gap-6 group">
                ...
                <span className="text-[10px] uppercase tracking-[0.5em] font-black text-gold/60 group-hover:text-gold transition-colors">5-Star Luxury</span>
            </div>
            <div className="flex flex-col items-center gap-6 group">
                <Scissors className="h-12 w-12 text-gold" />
                <span className="text-[10px] uppercase tracking-[0.5em] font-black text-gold/60 group-hover:text-gold transition-colors">Master Craft</span>
            </div>
            <div className="flex flex-col items-center gap-6 group">
                <Sparkles className="h-12 w-12 text-gold animate-pulse" />
                <span className="text-[10px] uppercase tracking-[0.5em] font-black text-gold/60 group-hover:text-gold transition-colors">Pure Luxury</span>
            </div>
        </div>
      </section>
    </div>
